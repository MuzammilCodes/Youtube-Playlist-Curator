...
//   playlists: Playlist[] = [];
//   videos: PlaylistItem[] = [];//previously video model
...
//         'https://www.googleapis.com/youtube/v3/videos?part=status&myRating=like',
...
//       // Fetch watch status for each video
...
//     try {
//       const response = await this.http.get<any>(
//         `https://www.googleapis.com/youtube/v3/videos?part=status&id=${videoIds}`,
//         { headers: this.getHeaders() }
//       ).toPromise();

//       // Update watch history with fetched video statuses
//       response.items?.forEach((item: any) => {
